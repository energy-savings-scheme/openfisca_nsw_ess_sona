...
class B1_electricity_savings(Variable):
    ...
    def formula(buildings, period, parameters):
        star_rating = buildings('washing_machine_star_rating', period)
        ...
        deemed_equipment_electricity_savings = (parameters(period).SONA.table_B1_1.deemed_equipment_electricity_savings[star_rating][washing_load])
        return deemed_equipment_electricity_savings


class B1_energy_savings(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'What are the energy savings for the Implementation?'
